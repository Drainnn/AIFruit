...
@interface chooseAddressEditController ()<UITableViewDelegate,UITableViewDataSource>{
    int flag;
}
...
@implementation chooseAddressEditController
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    __weak chooseAddressEditController *weakSelf = self;
    if (indexPath.section == 0) {
        ...
        static NSString *identifier = @"chooseAddressEditSaveCell";
        chooseAddressEditSaveCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
        if (!cell) {
            cell = [[[NSBundle mainBundle]loadNibNamed:identifier owner:nil options:nil]firstObject];
        }
